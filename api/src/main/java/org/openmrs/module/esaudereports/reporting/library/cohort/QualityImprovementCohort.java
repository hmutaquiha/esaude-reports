...
import org.openmrs.module.esaudereports.reporting.library.queries.quality.improvement.Category2CohortQueries;
...
public class QualityImprovementCohort {
	...
	public CohortDefinition hasEncountersAndInProgramNumeratorFirstPart() {
		...
		cd.addSearch("encountersPart1", ReportUtils.map(hasEncountersAndInProgramNumeratorSecondPart(),
		    "onOrAfter=${onOrAfter},onOrBefore=${onOrBefore},location=${location}"));
	...
	public CohortDefinition hasEncountersAndObs() {
		...
		cd.addSearch("encounter186and9", ReportUtils.map(
		    cohortLibrary.hasEncounter(FARMACIA, ADULTO_SEGUIMENTO, PEDIATRIA_SEGUIMENTO),
		    "onOrAfter=${onOrAfter},onOrBefore=${onOrBefore},location=${location}"));
		cd.addSearch("arvPlan",
		    ReportUtils.map(cohortLibrary.hasObs(arvPlan), "onOrAfter=${onOrAfter},onOrBefore=${onOrBefore}"));
		cd.addSearch("arvDrugUsed",
		    ReportUtils.map(cohortLibrary.hasObs(arvDrugUsed), "onOrAfter=${onOrAfter},onOrBefore=${onOrBefore}"));
	...
	public CohortDefinition hasEncounterPregnantAndNumberOfWeeks() {
		...
		cd.addParameter(new Parameter("onOrBefore", "End date", Date.class));
		cd.addParameter(new Parameter("location", "Location", Location.class));
		cd.addSearch("encounter5and6", ReportUtils.map(cohortLibrary.hasEncounter(ADULTO_INICIAL_A, ADULTO_SEGUIMENTO),
		    "onOrAfter=${onOrAfter},onOrBefore=${onOrBefore},location=${location}"));
		cd.addSearch("isPregnant",
		    ReportUtils.map(cohortLibrary.hasObs(pregnant, gestation), "onOrAfter=${onOrAfter},onOrBefore=${onOrBefore}"));
		cd.addSearch("pregnancyweeks",
		    ReportUtils.map(cohortLibrary.hasObs(pregnant, gestation), "onOrAfter=${onOrAfter},onOrBefore=${onOrBefore}"));
	...
	public CohortDefinition patientsWhoStartedARTInclusionPeriod() {
		...
		cd.addParameter(new Parameter("revisionEndDate", "Data Final de Revisão", Date.class));
		cd.addParameter(new Parameter("location", "Unidade Sanitária", Location.class));
		cd.setQuery(Category2CohortQueries.ART_START_INCLUSION_PERIOD_ART_SAMPLE);
	...
	public CohortDefinition patientsWithAtLeast3EnrollmentEvaluation3MonthsAfterARTStart() {
		...
		cd.setQuery(Category2CohortQueries.PATIENTS_WITH_AT_LEAST_3_ENROLLMENT_EVALUATION_3_MONTHS_AFTER_START_ART);
		return cd;
	}
	...
	public CohortDefinition pregnantSubrcribedOnART() {
		...
		cohort.setQuery(Category2CohortQueries.PREGNANTS_ENROLLED_ART_SERVICE);
		return cohort;
	}
